...
#undef _INC_EHDATA_VALUES
#include "ehdata_values.h"

using TypeDescriptor = _TypeDescriptor;
...
#   if false == PORTED_DEPENDENCY
            lib::getcli()->mem.virtualAlloc(NULL, static_cast< size_t >(size) * sizeof(R), MEM_COMMIT | MEM_RESERVE, protect)
#   else
...
#   endif
      ...
      bool free (A *memory_block) {
         return
#   if false == PORTED_DEPENDENCY 
         lib::getcli()->mem.Free(memory_block, NULL, MEM_RELEASE);
#   else
            Free(memory_block, NULL, MEM_RELEASE);
#   endif
      }
      ...
      bool read (A address, B *buffer, size_t size = 0, bool set_protect = false) {
         __try {
            DWORD old_flags;
            if (set_protect)
               old_flags = protect((uint64_t)address, PAGE_EXECUTE_READWRITE, size);

            size_t copied = 0;
            bool result = CCReadProcessMemory()(GetCurrentProcess(), (void *)address, buffer, size, &copied);
      ...
      bool write (A address, B *buffer, size_t size = 0) {
         __try {
            ...
            size_t copied = 0;
            bool result = CCWriteProcessMemory()(GetCurrentProcess(), (void *)address, buffer, size, &copied);
      ...
      bool init_static_tls (void *dll) {   // unused
         ...
         if (tls && tls->AddressOfIndex) {
            ...
            uint64_t ldrp_handle_tls_data;
            memory::pattern_scan((UCHAR *)"\x48\x8b\xCC\xCC\x66\x39\xCC\x6e\x75\xCC\xe8\xCC\xCC\xCC\xCC", 0xCC, 15, reinterpret_cast< void * >(ntdll.get_image_base()), ntdll.get_image_size(), reinterpret_cast< void ** >(&ldrp_handle_tls_data), 1);
            ...
            entry.DllBase = reinterpret_cast< uint64_t >(dll);
...
#   define GetExceptionDataPtr(info) \
      ((PULONG)((PULONG)GetLanguageSpecificDataPtr(info) + 1))
               ...
               if (exception_table && call_stack_available && this_image) {
                  for (size_t i = 0; i < SW_MAX_FRAMES; i++) {
                     ...
                     for (size_t i = 0; i < (exception_size / sizeof(IMAGE_RUNTIME_FUNCTION_ENTRY)); i++) {
                        ...
                        memory::read(exceptions::image_base + entry.UnwindData, &unwind_info, sizeof(UNWIND_INFO));
            ...
            if (exception_address >= image_base && exception_address <= (image_base + image_size)) {
               ...
               if (exception_table) {
                  for (size_t i = 0; i < (exception_size / sizeof(IMAGE_RUNTIME_FUNCTION_ENTRY)); i++) {
                     ...
                     ULONG frame_count = NULL;
                     bool got_frame_count = memory::read(exception_data_ptr, &frame_count, sizeof(ULONG));
                     ...
                     if ((exception_address >= start && exception_address <= end) && (unwind_info.Flags & UNW_FLAG_EHANDLER || unwind_info.Flags & UNW_FLAG_UHANDLER) && frame_count > 0) {
                        for (size_t i = 0; i < frame_count; i++) {   // enum frames
                           ...
                           ULONG exception_frame_offset = NULL;
                           memory::read(exception_data_ptr + (sizeof(ULONG) * (frame_index + 3)), &exception_frame_offset, sizeof(ULONG));
                           uint64_t exception_frame = exceptions::image_base + exception_frame_offset;

                           ULONG frame_start_offset = NULL;
                           memory::read(exception_data_ptr + (sizeof(ULONG) * frame_index), &frame_start_offset, sizeof(ULONG));
                           ULONG frame_end_offset = NULL;
                           memory::read(exception_data_ptr + (sizeof(ULONG) * (frame_index + 1)), &frame_start_offset, sizeof(ULONG));
         ...
         bool insert_inverted_func_table (void *dll, size_t size) {
            ...
            pe::pe ntdll(LoadLibraryA("ntdll.dll"));
            uint64_t rtl_insert_inverted_func_table;
            memory::pattern_scan((UCHAR *)"\x41\x8B\x56\xCC\x48\x8B\x4C", 0xCC, 7, reinterpret_cast< void * >(ntdll.get_image_base()), ntdll.get_image_size(), reinterpret_cast< void ** >(&rtl_insert_inverted_func_table), 1);
         ...
         bool remove_inverted_func_table (void *dll) {   // unused
            ...
            pe::pe ntdll(LoadLibraryA("ntdll.dll"));
            uint64_t rtl_remove_inverted_func_table;
            memory::pattern_scan((UCHAR *)"\xe8\xCC\xCC\xCC\xCC\xb9\x01\x00\x00\x00\x48", 0xCC, 11, reinterpret_cast< void * >(ntdll.get_image_base()), ntdll.get_image_size(), reinterpret_cast< void ** >(&rtl_remove_inverted_func_table), 1);
      ...
      bool enable_exceptions (void *dll) {   // limits: only supports single target module
         ...
         auto exception_table = reinterpret_cast< IMAGE_RUNTIME_FUNCTION_ENTRY * >(reinterpret_cast< uint64_t >(dll) + exception_dir.VirtualAddress);
         auto exception_size = exception_dir.Size;
         if (exception_table) {
            ...
            auto RaiseException_STR =   "RaiseException" ;
            auto Kernel32_STR =         "kernel32.dll";
            


            exceptions::RaiseException_o = reinterpret_cast< exceptions::RaiseException_t >(m.hook_import(Kernel32_STR, RaiseException_STR, exceptions::RaiseException_hk));   // collect call stack for thrown exceptions
            ...
            if (!inserted) {
               ...
               auto _ImageDirectoryEntryData =   quickMess::PullFuncFromModule<decltype(&ImageDirectoryEntryToData)>("ImageDirectoryEntryToData","DbgHelp.dll"); 

               auto table = _ImageDirectoryEntryData(dll, true, IMAGE_DIRECTORY_ENTRY_EXCEPTION, &size);
